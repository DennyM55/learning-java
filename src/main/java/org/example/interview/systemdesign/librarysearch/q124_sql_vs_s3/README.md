...
When would you use SQL storage versus S3-style object storage?

# Explanation

## The original problem

The original interview problem is about `q124_sql_vs_s3`. The interviewer is asking whether you can explain SQL Vs S3 clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
```text
Problem -> Identify SQL Vs S3 -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about SQL Vs S3. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- SQL Vs S3 = topic for this stable question ID
...
Use SQL Vs S3 when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use SQL Vs S3 only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Use SQL for structured records, relationships, constraints, and queryable metadata. Use object storage such as S3 for large immutable file bytes. Many systems use both together.

# Interview Answer

SQL Vs S3 is mainly about solving this problem: When would you use SQL storage versus S3-style object storage? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem SQL Vs S3 solves, then explain the mechanism.
