...
The original interview problem is about `q091_password_storage`. The interviewer is asking whether you can explain Password Storage clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
A password must not be stored as plain text. Store a salted one-way hash. A salt is random data added before hashing so equal passwords do not produce equal stored values.

## Step-by-step flow

```text
Problem -> Identify Password Storage -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Password Storage. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Password Storage = topic for this stable question ID
...
Use Password Storage when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Password Storage only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Password Storage by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Password Storage is mainly about solving this problem: How should passwords be stored securely? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Password Storage solves, then explain the mechanism.
