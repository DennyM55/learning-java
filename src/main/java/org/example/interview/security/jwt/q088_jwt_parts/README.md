...
The original interview problem is about `q088_jwt_parts`. The interviewer is asking whether you can explain JWT Parts clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
A JWT has three parts: header, payload, and signature. The header names the token type and signing algorithm. The payload contains claims. The signature proves the token was not changed after signing.

## Step-by-step flow

```text
Problem -> Identify JWT Parts -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about JWT Parts. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- JWT Parts = topic for this stable question ID
...
Use JWT Parts when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use JWT Parts only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare JWT Parts by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

JWT Parts is mainly about solving this problem: What are the parts of a JWT? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem JWT Parts solves, then explain the mechanism.
