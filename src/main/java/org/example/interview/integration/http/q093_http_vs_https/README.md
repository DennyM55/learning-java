...
The original interview problem is about `q093_http_vs_https`. The interviewer is asking whether you can explain HTTP Vs HTTPS clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
The key idea in HTTP Vs HTTPS is to explain the purpose first, then the mechanism, then the trade-off.

A practical answer should connect HTTP Vs HTTPS to a small example, mention what can go wrong, and describe how you would verify that the solution works.

## Step-by-step flow

```text
Problem -> Identify HTTP Vs HTTPS -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about HTTP Vs HTTPS. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- HTTP Vs HTTPS = topic for this stable question ID
...
Use HTTP Vs HTTPS when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use HTTP Vs HTTPS only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

HTTP sends data without built-in encryption. HTTPS is HTTP over TLS, where TLS means Transport Layer Security, the protocol that encrypts traffic and verifies server identity.

# Interview Answer

HTTP Vs HTTPS is mainly about solving this problem: What is the difference between HTTP and HTTPS? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem HTTP Vs HTTPS solves, then explain the mechanism.
