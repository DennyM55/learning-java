...
The original interview problem is about `q098_grpc`. The interviewer is asking whether you can explain GRPC clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
The key idea in GRPC is to explain the purpose first, then the mechanism, then the trade-off.

A practical answer should connect GRPC to a small example, mention what can go wrong, and describe how you would verify that the solution works.

## Topic-specific interview detail

gRPC is a remote procedure call framework that usually uses Protocol Buffers for compact typed messages. Protocol Buffers are a schema-based binary format.

## Step-by-step flow

```text
Problem -> Identify GRPC -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about GRPC. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- GRPC = topic for this stable question ID
...
Use GRPC when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use GRPC only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare GRPC by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

GRPC is mainly about solving this problem: What is gRPC? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem GRPC solves, then explain the mechanism.
