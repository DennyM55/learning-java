...
The original interview problem is about `q116_kubernetes_and_docker`. The interviewer is asking whether you can explain Kubernetes And Docker clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
The key idea in Kubernetes And Docker is to explain the purpose first, then the mechanism, then the trade-off.

A practical answer should connect Kubernetes And Docker to a small example, mention what can go wrong, and describe how you would verify that the solution works.

## Step-by-step flow

```text
Problem -> Identify Kubernetes And Docker -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Kubernetes And Docker. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Kubernetes And Docker = topic for this stable question ID
...
Use Kubernetes And Docker when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Kubernetes And Docker only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Kubernetes And Docker by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Kubernetes And Docker is mainly about solving this problem: How do Kubernetes and Docker work together? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Kubernetes And Docker solves, then explain the mechanism.
