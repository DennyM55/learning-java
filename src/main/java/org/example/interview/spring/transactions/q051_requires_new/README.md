...
What does REQUIRES_NEW mean in Spring transaction propagation?

# Explanation

## The original problem

The original interview problem is about `q051_requires_new`. The interviewer is asking whether you can explain Requires New clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
REQUIRES_NEW suspends the existing transaction and starts a separate new transaction. The inner transaction can commit or roll back independently from the outer transaction, but this can surprise people if they expect one all-or-nothing unit.

## Step-by-step flow

```text
Problem -> Identify Requires New -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Requires New. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Requires New = topic for this stable question ID
...
Use Requires New when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Requires New only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Requires New by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Requires New is mainly about solving this problem: What does REQUIRES_NEW mean in Spring transaction propagation? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Requires New solves, then explain the mechanism.
