...
The original interview problem is about `q046_transactional_internal_working`. The interviewer is asking whether you can explain Transactional Internal Working clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
Spring @Transactional usually works through a proxy. The proxy starts a transaction before the method, calls the real method, then commits or rolls back depending on success or failure.

## Step-by-step flow

```text
Problem -> Identify Transactional Internal Working -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Transactional Internal Working. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Transactional Internal Working = topic for this stable question ID
...
Use Transactional Internal Working when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Transactional Internal Working only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Transactional Internal Working by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Transactional Internal Working is mainly about solving this problem: How does @Transactional work internally in Spring? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Transactional Internal Working solves, then explain the mechanism.
