...
When does a Spring transaction roll back?

# Explanation

## The original problem

The original interview problem is about `q047_transaction_rollback`. The interviewer is asking whether you can explain Transaction Rollback clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
By default, Spring rolls back on unchecked exceptions, which are RuntimeException and Error types. Checked exceptions do not roll back by default unless rollback rules are configured.

## Step-by-step flow

```text
Problem -> Identify Transaction Rollback -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Transaction Rollback. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Transaction Rollback = topic for this stable question ID
...
Use Transaction Rollback when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Transaction Rollback only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Transaction Rollback by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Transaction Rollback is mainly about solving this problem: When does a Spring transaction roll back? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Transaction Rollback solves, then explain the mechanism.
