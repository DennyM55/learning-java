...
Can an asynchronous failure roll back the original transaction?

# Explanation

## The original problem

The original interview problem is about `q050_async_rollback_behavior`. The interviewer is asking whether you can explain Async Rollback Behavior clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
```text
Problem -> Identify Async Rollback Behavior -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Async Rollback Behavior. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Async Rollback Behavior = topic for this stable question ID
...
Use Async Rollback Behavior when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Async Rollback Behavior only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Async Rollback Behavior by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Async Rollback Behavior is mainly about solving this problem: Can an asynchronous failure roll back the original transaction? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Async Rollback Behavior solves, then explain the mechanism.
