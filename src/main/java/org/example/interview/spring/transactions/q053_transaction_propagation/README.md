...
What are Spring transaction propagation modes?

# Explanation

## The original problem

The original interview problem is about `q053_transaction_propagation`. The interviewer is asking whether you can explain Transaction Propagation clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
```text
Problem -> Identify Transaction Propagation -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Transaction Propagation. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Transaction Propagation = topic for this stable question ID
...
Use Transaction Propagation when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Transaction Propagation only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Transaction Propagation by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Transaction Propagation is mainly about solving this problem: What are Spring transaction propagation modes? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Transaction Propagation solves, then explain the mechanism.
