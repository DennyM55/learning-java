...
What is Mockito, and how do you use it in unit tests?

# Explanation

## The original problem

The original interview problem is about `q060_mockito`. The interviewer is asking whether you can explain Mockito clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
The key idea in Mockito is to explain the purpose first, then the mechanism, then the trade-off.

A practical answer should connect Mockito to a small example, mention what can go wrong, and describe how you would verify that the solution works.

## Topic-specific interview detail

Mockito creates test doubles. A test double is a fake replacement for a real dependency. Use it to test service logic without calling a real database, message broker, or HTTP service.

## Step-by-step flow

```text
Problem -> Identify Mockito -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Mockito. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Mockito = topic for this stable question ID
...
Use Mockito when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Mockito only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Mockito by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Mockito is mainly about solving this problem: What is Mockito, and how do you use it in unit tests? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Mockito solves, then explain the mechanism.
