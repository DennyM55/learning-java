...
How does an after-commit event work in Spring?

# Explanation

## The original problem

The original interview problem is about `q055_after_commit_event`. The interviewer is asking whether you can explain After Commit Event clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
```text
Problem -> Identify After Commit Event -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about After Commit Event. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- After Commit Event = topic for this stable question ID
...
Use After Commit Event when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use After Commit Event only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare After Commit Event by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

After Commit Event is mainly about solving this problem: How does an after-commit event work in Spring? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem After Commit Event solves, then explain the mechanism.
