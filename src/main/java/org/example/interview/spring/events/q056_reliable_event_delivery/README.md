...
How do you design reliable event delivery after a database transaction?

# Explanation

## The original problem

The original interview problem is about `q056_reliable_event_delivery`. The interviewer is asking whether you can explain Reliable Event Delivery clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
Reliable event delivery often uses the outbox pattern. The service writes business data and an outbox event in the same database transaction. A separate publisher reads unsent events, publishes them, and marks them sent.

## Step-by-step flow

```text
Problem -> Identify Reliable Event Delivery -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Reliable Event Delivery. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Reliable Event Delivery = topic for this stable question ID
...
Use Reliable Event Delivery when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Reliable Event Delivery only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Reliable Event Delivery by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Reliable Event Delivery is mainly about solving this problem: How do you design reliable event delivery after a database transaction? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Reliable Event Delivery solves, then explain the mechanism.
