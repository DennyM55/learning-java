...
What is the role of controller, service, and repository layers in Spring Boot?

# Explanation

## The original problem

The original interview problem is about `q057_controller_service_repository`. The interviewer is asking whether you can explain Controller Service Repository clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
The key idea in Controller Service Repository is to explain the purpose first, then the mechanism, then the trade-off.

A practical answer should connect Controller Service Repository to a small example, mention what can go wrong, and describe how you would verify that the solution works.

## Step-by-step flow

```text
Problem -> Identify Controller Service Repository -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Controller Service Repository. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Controller Service Repository = topic for this stable question ID
...
Use Controller Service Repository when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Controller Service Repository only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Controller Service Repository by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Controller Service Repository is mainly about solving this problem: What is the role of controller, service, and repository layers in Spring Boot? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Controller Service Repository solves, then explain the mechanism.
