...
The original interview problem is about `q059_entity_vs_dto`. The interviewer is asking whether you can explain Entity Vs DTO clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
An entity is an object mapped to a database table. A DTO, meaning Data Transfer Object, is an object designed to carry data across an API boundary.

DTOs matter because they prevent leaking database structure, lazy-loading behavior, and internal fields directly to clients.

## Step-by-step flow

```text
Problem -> Identify Entity Vs DTO -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about Entity Vs DTO. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- Entity Vs DTO = topic for this stable question ID
...
Use Entity Vs DTO when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use Entity Vs DTO only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare Entity Vs DTO by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

Entity Vs DTO is mainly about solving this problem: What is the difference between an entity and a DTO? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem Entity Vs DTO solves, then explain the mechanism.
