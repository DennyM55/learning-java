...
The original interview problem is about `q085_sql_optimization`. The interviewer is asking whether you can explain SQL Optimization clearly, apply it to a real situation, and recognize the mistakes that make the answer unsafe or incomplete.
...
The key idea in SQL Optimization is to explain the purpose first, then the mechanism, then the trade-off.

A practical answer should connect SQL Optimization to a small example, mention what can go wrong, and describe how you would verify that the solution works.

## Topic-specific interview detail

SQL optimization usually checks execution plans, indexes, joins, filters, selected columns, and data volume. An execution plan is the database's chosen strategy for running the query.

## Step-by-step flow

```text
Problem -> Identify SQL Optimization -> Choose approach -> Explain trade-off -> Verify result
```

## Practical example

Imagine a backend interview asks about SQL Optimization. A strong answer should not jump directly to syntax. It should explain the real problem, choose the simplest correct approach, and mention the limitation that would matter in production.
...
- SQL Optimization = topic for this stable question ID
...
Use SQL Optimization when the problem matches its purpose and the trade-offs are acceptable for the system you are building.

## When not to use it

Do not use SQL Optimization only because it sounds advanced. Avoid it when a simpler design is clearer, safer, or easier to operate.

## Comparison

Compare SQL Optimization by asking: what problem does it solve, what cost does it add, how does it fail, and what alternative would I use when requirements are different?

# Interview Answer

SQL Optimization is mainly about solving this problem: How do you optimize a SQL query? I would start by explaining why the problem exists, then describe the mechanism in simple steps, and finally mention the trade-off. In a real project I would choose it only when it makes the code or system clearer, safer, or more scalable. I would also verify the behavior with a small example, tests, logs, or metrics depending on the topic.
...
Start with the problem SQL Optimization solves, then explain the mechanism.
